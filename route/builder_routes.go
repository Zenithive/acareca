...
func RegisterBuilderRoutes(
	...
	// Form routes
	formGroup := v1.Group("/form", middleware.Auth(cfg), middleware.RequireActiveSubscription(dbConn), middleware.AuditContext())
	...
	entryGroup := v1.Group("/entry", middleware.Auth(cfg), middleware.RequireActiveSubscription(dbConn), middleware.AuditContext())
