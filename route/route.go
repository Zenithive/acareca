...
func RegisterRoutes(r *gin.Engine, cfg *config.Config, events sharedEvents.IEvent) (audit.Service, *sharednotification.Hub, notification.Repository, notification.Service, *worker.Consumer, *userSubscription.ExpiryWorker) {
	...
	authSvc := auth.NewService(authRepo, cfg, dbConn, practitionerSvc, auditSvc, invitationSvc, practitionerRepo, accountantSvc, adminSvc, fileRepo, preferenceSvc, adminSubscriptionSvc, userSubscriptionSvc, userSubscriptionRepo, coaRepo)
