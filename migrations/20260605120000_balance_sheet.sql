...
SELECT 
    ...
    CASE
        WHEN coa.code = 600 THEN COALESCE(fev.gross_amount, 0)
        WHEN coa.code = 820 THEN COALESCE(fev.gst_amount, 0)
        WHEN coa.classification IN ('Contra-Asset', 'Contra-Equity') THEN -COALESCE(fev.net_amount, 0)
...
FROM tbl_form_entry fe
    ...
    JOIN (
        SELECT DISTINCT ON (entry_id, COALESCE(form_field_id::text, coa_id::text))
            id, entry_id, form_field_id, coa_id,
            net_amount, gst_amount, gross_amount, description
...
WHERE fe.status = 'SUBMITTED'
    ...
    AND (
        ...
        OR (ff.id IS NULL AND fev.coa_id IS NOT NULL)
    );

COMMENT ON VIEW vw_balance_sheet_line_items IS 
'Balance Sheet transactions: Bank(600)=gross, GST(820)=gst_amount, Contra=-net, Others=net';
...
COMMENT ON VIEW vw_balance_sheet_summary IS 
'Aggregated Balance Sheet balances per account';
