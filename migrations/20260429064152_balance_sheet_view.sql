...
SELECT
    ...
    -- COA information (from direct reference OR via form field)
    ...
    -- Owner fund accounts (880, 881, 970) handled by COA code
    -- Drawings (880): negative (reduces equity)
    -- Funds Introduced (881): positive (increases equity)
    -- Share Capital (970): positive (increases equity)
    CASE 
        WHEN coa.code = 880 THEN -COALESCE(fev.net_amount, 0)  -- Drawings
        WHEN coa.code = 881 THEN COALESCE(fev.net_amount, 0)   -- Funds Introduced
        WHEN coa.code = 970 THEN COALESCE(fev.net_amount, 0)   -- Share Capital
...
JOIN (
    SELECT DISTINCT ON (entry_id, COALESCE(form_field_id::text, coa_id::text))
        id, entry_id, form_field_id, coa_id, net_amount, gst_amount, gross_amount, description
    FROM tbl_form_entry_value
    WHERE updated_at IS NULL
    ORDER BY entry_id, COALESCE(form_field_id::text, coa_id::text), created_at DESC
...
WHERE fe.status = 'SUBMITTED'
  ...
  AND (ff.is_formula = FALSE OR ff.id IS NULL)
